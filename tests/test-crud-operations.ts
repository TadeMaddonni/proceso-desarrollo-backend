...
async function testCrudOperations(): Promise<void> {
  try {
    ...
    console.log(`   📋 Registros de historial encontrados: ${historiales.length}`);    // 2. TEST: Verificar relaciones - Usuario con Zona y Deporte
    ...
    const usuarioCompleto = await Usuario.findByPk(nuevoUsuario.id, {
      include: [
        { model: Zona, as: 'zona' },
        { model: Deporte, as: 'deporte' },
        { model: Historial, as: 'historiales' }
      ]
    }) as any;
    ...
    if (usuarioCompleto) {
      ...
      console.log(`   📋 Historiales: ${usuarioCompleto.historiales?.length || 0}`);
    }
